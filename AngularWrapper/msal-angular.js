    ...
    if (angular) {
        ...
        MsalModule.provider('msalAuthenticationService', function () {
            ...
            var updateDataFromCache = function (scopes) {
                ...
                var user = _msal.getUser() || { userName: '' };
                _oauthData.userName = user.name;
                _oauthData.idToken = user.idToken;
                _oauthData.loginError = cacheResult == null ? "" : cacheResult.error;
            };
            ...
            this.$get = ['$rootScope', '$window', '$q', '$location', '$timeout', '$injector', function ($rootScope, $window, $q, $location, $timeout, $injector) {
                ...
                var processHash = function (hash, event, $window) {
                    ...
                    if (_msal.isCallback(hash)) {
                        ...
                        if (_msal._openedWindows.length > 0 && _msal._openedWindows[_msal._openedWindows.length - 1].opener
                            && _msal._openedWindows[_msal._openedWindows.length - 1].opener.msal) {
                            var mainWindow = _msal._openedWindows[_msal._openedWindows.length - 1].opener;
                        ...
                        if (requestInfo.stateMatch) {
                            if (requestInfo.requestType === "RENEW_TOKEN") {
                                ...
                                if (_oauthData.userName) {
                                    ...
                                    $rootScope.$broadcast('msal:loginFailure', errorDescription, error);
                                }
                ...
                var loginHandler = function (loginStartPage, routeProtectionConfig) {
                    ...
                    if (routeProtectionConfig.popUp) {
                        _msal.loginPopup(routeProtectionConfig.scopes, routeProtectionConfig.extraQueryParameters);
                    }
                    else {
                        _msal.loginRedirect(routeProtectionConfig.scopes, routeProtectionConfig.extraQueryParameters);
                    }

                };

                function isAnonymousEndpoint(url) {
                    if (_msal && _msal._anonymousEndpoints) {
                        for (var i = 0; i < _msal._anonymousEndpoints.length; i++) {
                            if (url.indexOf(_msal._anonymousEndpoints[i]) > -1) {
                                return true;
                            }
                        }
                    }
                    ...
                    if (nextRoute && nextRoute.$$route) {
                        ...
                        if (requireLogin) {
                            ...
                            if (nextRouteUrl && !isAnonymousEndpoint(nextRouteUrl)) {
                                _msal._anonymousEndpoints.push(nextRouteUrl);
                            }
                        }
                    }
                };
                        ...
                        for (var i = 0; i < states.length; i++) {
                            ...
                            if (requireLogin) {
                                ...
                                if (nextStateUrl && !isAnonymousEndpoint(nextStateUrl)) {
                                    _msal._anonymousEndpoints.push(nextStateUrl);
                                }
                            }
                        }
                    }
                ...
                return {
                    ...
                    removeItem: function (key) {
                        _msal._cacheStorage.removeItem(key);
                    }
                };
            }];
        });
            ...
            return {
                request: function (config) {
                    if (config) {
                        ...
                        var scopes = authService.getScopesForEndpoint(config.url);
                        var $state = $injector.get('$state').current;
                        var routeProtectionConfig = window.msal.routeProtectionConfig;

                        authService.verbose('Url: ' + config.url + ' maps to scopes: ' + scopes);
                        if (scopes === null) {
                            return config;
                        }
                        var cacheResult = authService.getCachedToken(scopes, authService.getUser());
